...
void init()
{
    ...
    // 建立顶点变量vPosition在着色器中的索引 同时指定vPosition变量的数据解析格式
     // vPosition变量在着色器程序中的位置索引。注意vPosition这个变量名是和shader程序共享的！你可以在shader程序中找到vPosition这个名称。
     //所以这里用字符串给出。
    // GLuint vlocation = glGetAttribLocation(program, "vPosition");   
    //把vlocation作为顶点属性之一。（视GPU实际情况，顶点属性可以enable最多16个或更多。属性可以是例如位置、颜色、纹理坐标、甚至彩蛋（你往顶点上藏一句话也不是不可以））
    //layout location=0
    glEnableVertexAttribArray(0);
    // 指定vao中的vPosition变量的数据解析格式.
    // 负责指定shader中，如何传入顶点变量。
    // vlocation是待传入的变量在shader源程序中中的索引。
    // 3是3个数一组作为一个变量，/每个的格式都是GL_FLOAT。
    //GL_FALSE是禁用归一化。
    //0是连续的属性之间的偏移量。（属性之间要间隔多远）
    //(GLvoid*)0是base。也就是约定了从哪里开始读取数据，往gpu中传送。这里的位置默认是从vbo中找，因为vao只是用来解释vbo中数据的。
    //layout location=0
    ...
    glEnableVertexAttribArray(1);
        //这里，base是(GLvoid*)(sizeof(glm::vec3) * points.size()。
        //layout location=1
