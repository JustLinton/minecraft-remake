...
namespace Plankton
{
    ...
    {
        private PlanktonVertexList _vertices;//私有成员，不能被外部调用
        private PlanktonHalfEdgeList _halfedges;//私有成员，不能被外部调用
        private PlanktonFaceList _faces;//私有成员，不能被外部调用
        ...
        /// </summary>
        public PlanktonVertexList Vertices //能被外部调用的顶点列表属性，第一次调用能创建顶点列表，并把本实例对象代入
        {
            get { return _vertices ?? (_vertices = new PlanktonVertexList(this)); }//使用了可空类型运算符
        }
        
        /// <summary>
        /// Gets access to the <see cref="PlanktonHalfedgeList"/> collection in this mesh.
        /// </summary>
        public PlanktonHalfEdgeList Halfedges //能被外部调用的半边列表属性，第一次调用能创建半边列表，并把本实例对象代入
        {
            get { return _halfedges ?? (_halfedges = new PlanktonHalfEdgeList(this)); }//使用了可空类型运算符
        }
        
        /// <summary>
        /// Gets access to the <see cref="PlanktonFaceList"/> collection in this mesh.
        /// </summary>
        public PlanktonFaceList Faces //能被外部调用的面片列表属性，第一次调用能创建面片列表，并把本实例对象代入
        {
            get { return _faces ?? (_faces = new PlanktonFaceList(this)); }//使用了可空类型运算符
        }
        #endregion
        ...
        /// </summary>
        public double Volume() //使用行列式计算体积（与原点构成四面体）
        {
            double VolumeSum = 0;
