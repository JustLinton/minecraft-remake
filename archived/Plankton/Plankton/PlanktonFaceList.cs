...
namespace Plankton
{
    ...
    public class PlanktonFaceList : IEnumerable<PlanktonFace>
    {
        ...
        /// </summary>
        /// <param name="owner">网格对象The <see cref="PlanktonMesh"/> to which this list of half-edges belongs.</param>
        ...
        /// 很重要的一个方法，通过顶点索引添加面片，添加的面片可以是多边形
        /// </summary>
        ...
        public int AddFace(IEnumerable<int> indices)
        {
            ...
            int[] array = indices.ToArray(); // using Linq for convenience，使用数组能方便使用Linq

            var hs = _mesh.Halfedges;//类型应为PlanktonHalfEdgeList，用var可能是忘了类型
            var vs = _mesh.Vertices;//类型应为PlanktonVertexList，用var可能是忘了类型
            int n = array.Length;
            ...
            int[] loop = new int[n];//每个面片的边缘都是由半边构成的有向多边形环路
            bool[] is_new = new bool[n];//有向多边形环路的每条边都能用半边来表示
            ...
            for (int i = 0, ii = 1; i < n; i++, ii++, ii %= n)
            {
                ...
                //查找halfedge(v1,v2)是否已存在
                int h = hs.FindHalfedge(v1, v2);//通过首尾顶点查询半边索引
                if (h < 0)
                ...
                else if (hs[h].AdjacentFace > -1)
                {
                    ...
                    //虽然该半边的起点的出半边是边界半边（可以用它创建新的面片），然而该半边并不是该边界半边，从而必定已被两个面片共用
                    //故不能用它创建新的面片。事实上这里的情况包含前面判断的情况，只是增加前面的判断能及早判断那种情况罢了
                    return -1;
                }
                else
                    loop[i] = h;//该半边是边界半边，能创建新面片，把它放在环路中
            ...
            for (int i = 0, ii = 1; i < n; i++, ii++, ii %= n)
            {
                ...
                {
                    int v1 = array[i], v2 = array[ii];
                    loop[i] = hs.AddPair(v1, v2, this.Count);//必须成对添加半边，成对的半边索引是相邻的
                }
                else
                {
                    // Link existing halfedge to new face
                    hs[loop[i]].AdjacentFace = this.Count;//添加第一条边的邻接面为该邻接面
                }
            }
            
            // Link halfedges，通过不同的情况，确定半边的下一条边和上一条边
            for (int i = 0, ii = 1; i < n; i++, ii++, ii %= n)
            {
                //int v1 = array[i];
                int v2 = array[ii];
                ...
                if (id > 0) // At least one of the halfedge pairs is new...
                {
                    ...
                    switch (id)//通过两条相邻半边的新旧情况，决定外围半边的下/上一条边
                    {         
                        ...
                        case 4: // both are new (non-manifold vertex)
                            ...
                            //非流形顶点，构成的面片与新面片没有共享边
                            //非流形顶点是允许的，只是不允许非流形半边
                    ...
                    if (outer_prev > -1 && outer_next > -1)//为什么不用makeConsecutive方法？
                    {
                        hs[outer_prev].NextHalfedge = outer_next;
                        hs[outer_next].PrevHalfedge = outer_prev;
                    }
                    
                    // Link inner halfedges，对面片环路半边进行链接
                    ...
                    {
                        vs[v2].OutgoingHalfedge = loop[i] + 1;//保证边界半边必为出半边（反之不一定）
                    }
                }
                else // both old (non-manifold vertex trickery below)，id为0的时候，即两条相邻半边都为旧半边
                {
                    ...
                    if (vs[v2].OutgoingHalfedge == loop[ii])//第2个顶点的出半边刚好是下一条边（常见情况）
                    {
                        foreach (int h in hs.GetVertexCirculator(loop[ii]).Skip(1))
                        {
                            if (hs[h].AdjacentFace < 0)
                            {
                                vs[v2].OutgoingHalfedge = h;//寻找边界半边，试图以该半边为新的出半边
                                break;                      //若该顶点已封闭，则不会找到边界半边
                            }
                        }
                    }
                    ...
                    if (hs[loop[i]].NextHalfedge != loop[ii] || hs[loop[ii]].PrevHalfedge != loop[i])//考虑三个不共享边的面片共享同一个顶点的情况
                    {
            ...
            PlanktonFace f = new PlanktonFace() { FirstHalfedge = loop[0] };//最后才创建新的面片，并赋予首半边
            
            return this.Add(f);//把面片添加进面片列表中
        }
        ...
        {
            return faces.Select(f => this.AddFace(f)).ToArray();//使用Linq
        }
